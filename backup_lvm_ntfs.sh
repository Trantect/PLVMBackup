...
DST_FILE=${DST_DIR}/${LVM_GROUP}-`echo ${SNAPSHOT_DEV}|gawk -F'/' '{print $4}'`.mbr.lzo
echo $DST_FILE
dd if=${SNAPSHOT_DEV} bs=512 count=2048|lzop -c >${DST_FILE}
for A in ${PARTITION_PREFIX}*; do
	...
	DST_FILE=${DST_DIR}/`echo ${A}|gawk -F'/' '{print $4}'`.ntfsclone.lzo
	echo $DST_FILE
	#echo "partclone.${FSTYPE} -c -s ${A} | lzop -c >${DST_FILE}"
	#partclone.${FSTYPE} -c -s ${A} | lzop -c >${DST_FILE}
	echo "ntfsclone -f -s -o - ${A} | lzop -c >${DST_FILE}"
	ntfsclone -f -s -o - ${A} | lzop -c >${DST_FILE}
done
sleep 5
